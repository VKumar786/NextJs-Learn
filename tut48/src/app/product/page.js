...
const Product = () => {
  ...
  const handleDeleteProductById = async (idx) => {
    let res = await fetch(`http://localhost:3000/api/products/${idx}`, {
      method: 'DELETE'
    })
    res = await res.json()
          ...
          {arr.results.map((item, idx) => {
            ...
            return (
              <tr key={idx}>
                ...
                <td>
                  <button onClick={() => handleDeleteProductById(item._id)}>DELETE</button>
                </td>
              </tr>
            );
          })}
